...
export function AssessmentLibraryPage() {
  ...
  return (
    <div className="space-y-10">
      ...
      {assessments.isLoading ? <Loading /> : assessments.isError ? <p className="rounded-xl bg-rose-50 p-4 text-rose-700">Không tải được thư viện đánh giá.</p> : (
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {assessments.data?.map((assessment) => <AssessmentCard key={assessment.id} assessment={assessment} />)}
        </div>
      )}
