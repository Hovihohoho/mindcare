...
const initialMessages: ChatMessage[] = [
  { id: "welcome", role: "assistant", content: "Chào bạn! Mình là MindCare AI. Bạn muốn chia sẻ điều gì hôm nay?", createdAt: "Bây giờ" },
];
...
export function AiChatPage() {
  ...
  return (
    <Card className="grid min-h-[760px] overflow-hidden rounded-none lg:grid-cols-[280px_1fr]">
      <aside className="hidden border-r border-line bg-white p-5 lg:flex lg:flex-col"><div className="rounded-xl bg-slate-100 p-4"><p className="font-semibold">AI Trợ lý MindCare</p><p className="mt-1 text-xs text-emerald-600">● Đang trực tuyến</p></div><p className="mt-8 text-xs font-extrabold tracking-wider text-muted">TRÒ CHUYỆN GẦN ĐÂY</p><p className="mt-4 text-sm text-muted">AI Service chưa hỗ trợ lưu lịch sử hội thoại.</p><Button className="mt-auto w-full bg-emerald-700 hover:bg-emerald-800" leftIcon={<Plus className="size-4" />} onClick={() => setMessages(initialMessages)}>Cuộc hội thoại mới</Button></aside>
