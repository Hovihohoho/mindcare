    ...
    >
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge tone={item.writable ? "success" : "neutral"}>{item.writable ? "Đang mở" : "Chỉ đọc"}</Badge>
        </div>
        <p className="mt-2 flex items-center gap-2 text-sm text-slate-500">
          <MessageCircle className="size-4 shrink-0" aria-hidden="true" />
          Bắt đầu {new Date(item.openedAt).toLocaleString("vi-VN", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
            hour: "2-digit",
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              Các cuộc trao đổi với chuyên gia được lưu tại đây và không phụ thuộc vào lịch hẹn hay thanh toán.
            </p>
          </div>
        </div>
      </header>
        ...
        {!history.isLoading && !history.isError && conversations.length === 0 && (
          <div className="p-8">
            <EmptyState
              ...
              description="Khi bạn chọn trò chuyện từ hồ sơ chuyên gia, cuộc hội thoại sẽ xuất hiện tại đây."
            />
          </div>
        )}
